import 'package:examen_frontend_/src/data/api/ApiConfig.dart';
...
class MatriculaService {
  ...
  Future<Resource<List<Matricula>>> fetchMatriculas() async {
    try {
      Uri url = Uri.http(ApiConfig.API, '/api/alumnos');
  ...
  Future<Resource<Matricula>> fetchMatriculaDetail(int idMatricula) async {
    try {
      Uri url = Uri.http(ApiConfig.API, '/api/alumnos/$idMatricula');
      Map<String, String> headers = {"Content-Type": "application/json"};
      final response = await http.get(url, headers: headers);
  ...
  Future<Resource<Matricula>> updateMatricula(int idMatricula, Matricula matricula) async {
    try {
      Uri url = Uri.http(ApiConfig.API, '/api/alumnos/$idMatricula');
      Map<String, String> headers = {"Content-Type": "application/json"};
      ...
      if (response.statusCode == 200) {
        Matricula updatedMatricula = Matricula.fromJson(json.decode(response.body));
